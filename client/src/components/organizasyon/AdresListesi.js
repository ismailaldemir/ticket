...
import {
  getOrganizasyonAdresler,
  addOrganizasyonAdres,
  updateOrganizasyonAdres,
  deleteOrganizasyonAdres,
} from "../../redux/organizasyon/organizasyonSlice";
import { toast } from "react-toastify";
import KonumSec from "../common/KonumSec";

const AdresListesi = ({ organizasyonId }) => {
      ...
      if (organizasyonId) {
        try {
          ...
          const response = await dispatch(
            getOrganizasyonAdresler(organizasyonId)
          ).unwrap();
  ...
  const handleSaveAdres = async () => {
    try {
      ...
      if (seciliAdres) {
        ...
        setAdresler(
          adresler.map((adr) =>
            adr._id === seciliAdres._id ? { ...adr, ...formData } : adr
          )
        );
  ...
  const handleDeleteAdres = async () => {
    ...
    try {
      setLoading(true);
      await dispatch(deleteOrganizasyonAdres(seciliAdres._id)).unwrap();
            ...
            <Grid item xs={12} sm={6}>
              <FormControl fullWidth>
                ...
                >
                  ...
                  <MenuItem value="Sevkiyat">Sevkiyat</MenuItem>
                  <MenuItem value="Genel Merkez">Genel Merkez</MenuItem>
