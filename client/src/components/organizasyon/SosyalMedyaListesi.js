...
import {
  getOrganizasyonSosyalMedya,
  addOrganizasyonSosyalMedya,
  updateOrganizasyonSosyalMedya,
  deleteOrganizasyonSosyalMedya,
} from "../../redux/organizasyon/organizasyonSlice";
import { toast } from "react-toastify";

const SosyalMedyaListesi = ({ organizasyonId }) => {
    ...
    const fetchSosyalMedya = async () => {
      ...
      try {
        ...
        const response = await dispatch(
          getOrganizasyonSosyalMedya(organizasyonId)
        ).unwrap();

        // Veri kontrolü ve filtreleme
        ...
        setHesaplar(validData);
      } catch (error) {
        console.error("Sosyal medya bilgileri yüklenirken hata:", error);
        toast.error(error?.message || "Sosyal medya bilgileri yüklenemedi");
        setHesaplar([]);
      } finally {
        setLoading(false);
      }
  ...
  const handleSaveHesap = async () => {
    try {
      ...
      if (!formData.tur) {
        toast.error("Sosyal medya türü seçmelisiniz");
        setLoading(false);
        return;
      }
  ...
  const handleDeleteHesap = async () => {
    ...
    try {
      setLoading(true);
      await dispatch(deleteOrganizasyonSosyalMedya(seciliHesap._id)).unwrap();
